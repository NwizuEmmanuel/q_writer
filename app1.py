...
# --- Enhanced Styling for Prettier UI ---
from PySide6.QtGui import QPalette, QColor, QFont

class QuizBuilder(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Quiz Builder — PySide6 (Enhanced UI)')
        self.setStyleSheet('''
            QWidget {
                font-size: 14px;
    ...
    def remove_selected_question(self):
        ...
        if row < 0 or row >= len(self.questions):
            return
        confirm = QMessageBox.question(self, 'Remove', 'Delete selected question?', QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
